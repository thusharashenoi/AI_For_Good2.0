...
mkdir -p "$BUNDLE/engagement"
cp -r engagement/shared engagement/lambdas "$BUNDLE/engagement/"
# Strip tests/cache so unzipped bundle stays under Lambda's 250 MB limit.
find "$BUNDLE" -type d \( -name tests -o -name __pycache__ -o -name test -o -name testing -o -name .aws-sam \) -prune -exec rm -rf {} + 2>/dev/null || true
