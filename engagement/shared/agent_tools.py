...
class AgentTools:
    ...
    def check_eligibility(self, diabetes_insulin: Optional[bool] = None,
        ...
        if deferral["eligible"]:
            ...
            db.save_conversation(conv)
            existing = (conv.get("contextData") or {}).get("proposedAppointment")
            if (
                existing
                and existing.get("availabilityConfirmed")
    ...
    def book_appointment(self, request_id: Optional[str] = None,
        ...
        appt = {
            ...
            "bridgeId": req.get("bridgeId") or conv.get("bridgeId"),
            "createdAt": db.now_iso(),
        }
        db.save_appointment(appt)
        bridge_id = appt.get("bridgeId")
        if bridge_id and donor.get("donorId"):
            try:
                import sys
                ...
                from raktsetu import store as bridge_store
                bridge_store.confirm_bridge_donor(bridge_id, donor["donorId"], "PENDING")
